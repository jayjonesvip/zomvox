...
/*
 * ZomVox audio: DSP primitives
 *
 * Noise buffers, envelopes, filters, distortion curves, resonators, pitch helpers, and distance rolloff live here.
 ...
 * the live `AudioContext` â€” that is how this subsystem is verified without a
 ...
 *  white  â€” flat spectrum, the raw material of cracks and hiss
 *  pink   â€” -3 dB/oct (Paul Kellet's economy filter), city beds, tails
 *  brown  â€” -6 dB/oct leaky integrator, wind and rumble
 *  crackleâ€” sparse impulsive grains, debris and foliage
 */
...
/** Instant-attack exponential decay â€” the workhorse for transients. */
 ...
 * `asym` adds even harmonics â€” that is what gives a muzzle blast its "chuff"
...
function limiterCurve() {
  ...
  for (let i = 0; i < n; i++) {
    const x = (i / (n - 1)) * 2 - 1;
    // Cubic soft clip up to 0.66, then tanh â€” transparent below -6 dBFS.
    const a = Math.abs(x);
    let y;
    if (a < 0.66) y = x;
...
/** Semitone ratio â€” pitch jitter is expressed musically, not as a raw factor. */
function semis(n) {
