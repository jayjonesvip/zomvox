...
/*
 * ZomVox audio: foley, impacts, reloads, explosions, and UI
 *
 * Surface impacts, footsteps, reload mechanics, C4 blasts, body falls, hit markers, pickups, and heartbeat cues.
...
function surfaceImpact(actx, bank, rng, o = {}) {
  ...
  /* debris grains â€” chips, splinters, glass shards landing */
...
function footstep(actx, bank, rng, o = {}) {
  ...
  /* heel/toe transient â€” two contacts, milliseconds apart, is what reads as a
  ...
  /* scuff â€” the slide of the sole, longer when running */
  if (s.scuff > 0.05) {
  ...
  /* gear: sling swivels, mag pouches, buckles â€” only when moving fast */
...
function reloadPhase(actx, bank, rng, phase, o = {}) {
  ...
  switch (phase) {
    ...
    case 'magout': {
      ...
      // Empty magazine hitting the ground â€” polymer, not metal.
    ...
    default: {
      ...
      // Spring ring â€” the metallic "zing" behind the clack.
      metal(st + 0.065, [
...
function uiSound(actx, bank, rng, kind, o = {}) {
  ...
  switch (kind) {
    ...
    case 'grenade_warn': {
      // Three rising beeps â€” reads as "danger", not as a notification.
      for (let i = 0; i < 3; i++) {
        const bt = t0 + i * 0.14;
