...
/*
 * ZomVox audio: weapons
 *
 * Layered gunshot and dry-fire synthesis: transient, body, crack, tail, mechanical action, and distance boom.
 ...
 *
 *   1. TRANSIENT  sub-millisecond click â€” the pressure step. Gives the shot its
 ...
 *   3. CRACK      resonant band-passed noise around 1.5â€“3.5 kHz driven into
 *                 saturation. Calibre character lives here.
 *   4. MID        a short 500â€“900 Hz noise body that glues 2 and 3 together.
 *   5. TAIL       a broadband burst under a falling lowpass, fed hard into the
 *                 reverb send â€” this is what the *room* hears.
...
function weaponShot(actx, bank, rng, profile, o = {}) {
  ...
  // Per-shot jitter on top of the round-robin slot â€” the fine grain.
  ...
  /* ---- 2. body + sub -------------------------------------------- */
  {
    ...
    // Sub thump â€” this is the one that moves air; keep it out of the reverb.
  ...
  /* ---- 6. mechanical / bolt ------------------------------------- */
  // Only audible close up â€” a rifle 40 m away has no audible action noise, and
...
function bulletWhizz(actx, bank, rng, o = {}) {
  ...
  const g = gain(actx, 0);
  series(src, bp, g).connect(out);
  // The N-wave's apparent pitch drops sharply as the round passes â€” Doppler on
